...
========================
Laeuft via GitHub Actions jeden Montag abends deutscher Zeit, kurz nach
dem Bitvavo-Sparplan-Kauf (montags 20:50).
...
zur im Zeitplan hinterlegten Kaufzeit, ab dem 08.06.2026).
...
# Sparplan-Zeitplan. Jede Zeile: (erster Montag, letzter Montag|None, Stunde, Minute, Gebuehr).
# Zeile 1 = die 4 fixen Alt-Kaeufe (08:40, 0,40 %). Ab 06.07. laeuft es auf
# Bitvavo weiter: montags 20:50, Gebuehr 0,25 %.
SCHEDULE = [
    (dt.date(2026, 6, 8), dt.date(2026, 6, 29), 8, 40, 0.004),
    (dt.date(2026, 7, 6), None, 20, 50, 0.0025),
]
FIRST_MONDAY = SCHEDULE[0][0]
# Aktive (laufende) Kaufzeit = letzte Zeile im Zeitplan
BUY_HOUR = SCHEDULE[-1][2]
BUY_MIN = SCHEDULE[-1][3]
...
def price_at(ts_ms):
    """BTC/EUR-Preis zum Zeitpunkt ts_ms (Montag zur Kaufzeit, DE-Zeit).
...
def segment_for(d):
    """Liefert (Stunde, Minute, Gebuehr) fuer den Montag d, oder None."""
    for first, last, hour, minute, fee in SCHEDULE:
        if d >= first and (last is None or d <= last):
            return hour, minute, fee
    return None


def buy_mondays_until(today):
    """Alle Kauf-Montage bis heute als (Datum, Stunde, Minute, Gebuehr)."""
    out = []
    d = FIRST_MONDAY
    while d <= today:
        seg = segment_for(d)
        if seg is not None:
            out.append((d, seg[0], seg[1], seg[2]))
        d += dt.timedelta(days=7)
    return out


def buy_ts_ms(d, hour, minute):
    """UTC-Millisekunden fuer die Kaufzeit am Datum d (DST-sicher)."""
    local = dt.datetime(d.year, d.month, d.day, hour, minute, 0, tzinfo=TZ)
...
def main():
    ...
    # GitHub versucht es montagabends mehrfach. Gesendet wird beim ERSTEN Lauf,
    # der (a) montags, (b) nach 20:50 DE-Zeit liegt (nach dem Bitvavo-Kauf) und
    # (c) heute noch nicht gesendet hat. Der Sendetag wird in state/last_sent.txt
    # vermerkt, damit es bei mehreren Laeufen trotzdem nur EINE Nachricht gibt.
    if not force:
        ...
        if mins < BUY_HOUR * 60 + BUY_MIN:
            print(f"Vor 20:50 (aktuell {now_local:%H:%M}) -> warte auf den Kauf.")
            return
        if already_sent(today_iso):
            print(f"Fuer {today_iso} bereits gesendet -> uebersprungen.")
            return

    mondays = buy_mondays_until(today)
    if not mondays:
        print("Sparplan startet erst am 08.06.2026 - noch nichts zu senden.")
        return

    # Komplette Historie neu aufbauen (Gebuehr/Kaufzeit je nach Zeitplan-Segment)
    tranches = []
    cum_eur = 0.0
    cum_btc = 0.0
    for d, hour, minute, fee in mondays:
        price = price_at(buy_ts_ms(d, hour, minute))
        net = WEEKLY_EUR * (1 - fee)
        amount_btc = net / price
        cum_eur += WEEKLY_EUR
        cum_btc += amount_btc
        tranches.append({"date": d, "price": price, "btc": amount_btc,
                         "hour": hour, "minute": minute, "net": net})
    ...
    kurs_label = f"{this_m['hour']:02d}:{this_m['minute']:02d}"

    msg = (
        f"\u20BF <b>BTC-Sparplan \u2013 Montag {this_m['date']:%d.%m.%Y}</b>\n"
        f"\U0001F5D3\uFE0F <i>Woche {week_no}</i>\n"
        f"\n"
        f"\U0001F4B0 <b>Kurs ({kurs_label}):</b> {eur(this_m['price'])}\n"
        f"{cmp_line}\n"
        f"\n"
        f"\U0001F6D2 <b>Gekauft:</b> {btc(this_m['btc'])} BTC\n"
        f"<i>(fuer {eur(this_m['net'])} netto)</i>\n"
        f"\n"
